...
@section('title', 'Edit Fasilitas | Admin SMAN Pintar')

@section('content')
<div class="max-w-7xl mx-auto space-y-8">
    <div>
        <h2 class="text-4xl font-extrabold text-primary tracking-tight font-headline">Edit Fasilitas</h2>
    </div>

    <div class="bg-surface-container-lowest rounded-2xl shadow-sm p-8">
        <form action="{{ route('admin.about.facilities.update', $index) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')

            <div>
                <label class="text-xs font-bold uppercase text-tertiary block mb-2">Nama Fasilitas</label>
                <input type="text" name="title" value="{{ old('title', data_get($facility, 'title')) }}"
            ...
            <div>
                ...
                <div class="mb-3">
                    <img src="{{ asset('storage/' . data_get($facility, 'image')) }}" alt="Gambar fasilitas saat ini"
            ...
            <div>
                ...
                <textarea name="desc" rows="7"
                    class="w-full bg-surface-container border-none rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-primary" required>{{ old('desc', data_get($facility, 'desc')) }}</textarea>
